...
def test_admin_login_debug():
    ...
    try:
        # Connect to MongoDB to check admin users
        import pymongo
        from pymongo import MongoClient
    ...
    # Test 4: Check backend logs for authentication errors
    print("\n📋 Test 4: Checking Backend Logs for Authentication Errors")
    ...
    try:
        ...
        if result.returncode == 0:
            logs = result.stdout
            if 'login' in logs.lower() or 'auth' in logs.lower() or 'error' in logs.lower():
                print("📋 Recent backend logs (filtered for login/auth/error):")
                for line in logs.split('\n'):
                    if any(keyword in line.lower() for keyword in ['login', 'auth', 'error', 'exception']):
                        print(f"   {line}")
            else:
                print("📋 No login/auth/error entries found in recent logs")
        
    except Exception as e:
        print(f"❌ Error checking backend logs: {str(e)}")
    
    # Test 5: Test with case variations
    print("\n🔤 Test 5: Testing Case Variations")
...
if __name__ == "__main__":
    print("🚀 Starting ArgusAI CashOut Admin Login Debug Tests")
    
    # Run the admin login debug test
    test_admin_login_debug()
    
    print("\n✅ Admin Login Debug Tests Complete")
        200,
        data={
